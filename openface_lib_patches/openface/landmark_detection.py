...
class LandmarkDetector:
    def __init__(self, model_path: str, device: str = "cpu", device_ids: List[int] = [0]):
        if device == "cpu":
            device_id = "cpu"
            device_ids = [-1] # CPU mode needs -1
        elif device == "cuda":
            if not device_ids or device_ids[0] < 0:
                # [Patched] Auto-fix: if user forgot to pass device_ids but requested cuda, default to [0]
                device_ids = [0]
